  ...
  return (
    <div className="w-full flex flex-col gap-4" id="diff-viewer-root">
      ...
      {layout === 'inline' ? (
        ...
        >
          {/* Header */}
          <div className="flex items-center justify-between px-4 py-2 bg-slate-50 dark:bg-slate-950 border-b border-slate-100 dark:border-slate-800">
            <div className="flex items-center gap-2">
              <Eye className="w-3.5 h-3.5 text-indigo-500" />
              <span className="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest">Visualização Mesclada</span>
            </div>
            <button
              onClick={() => {
              ...
              className="flex items-center gap-1.5 px-3 py-1 text-xs text-slate-600 dark:text-slate-300 hover:text-indigo-600 dark:hover:text-indigo-400 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:border-indigo-150 rounded transition-all cursor-pointer font-semibold"
          ...
          {/* Render box */}
          <div className={`p-5 min-h-[160px] max-h-[500px] overflow-y-auto whitespace-pre-wrap break-words leading-relaxed ${fontClass} bg-white dark:bg-slate-900 transition-colors`}>
            {changes.map((change, idx) => {
              ...
              if (change.added) {
                return (
                  <span
                    key={idx}
                    data-diff-node={idx}
                    className={`bg-green-105 dark:bg-emerald-955/25 text-green-850 dark:text-emerald-305 px-1 py-[1.5px] rounded border-b border-green-200 dark:border-emerald-800/40 mx-0.5 inline bg-green-100 ${selectionStyle}`}
                    title="Adicionado"
                  >
                    {change.value}
                  </span>
              ...
              if (change.removed) {
                return (
                  <span
                    key={idx}
                    data-diff-node={idx}
                    className={`bg-red-105 dark:bg-red-955/35 text-red-850 dark:text-red-305 line-through decoration-red-300 dark:decoration-red-700 px-1 py-[1.5px] rounded mx-0.5 inline bg-red-100 ${selectionStyle}`}
                    title="Removido"
                  >
                    {change.value}
                  </span>
              ...
              return <span key={idx} className="text-slate-700 dark:text-slate-300">{change.value}</span>;
            })}
          </div>
        </motion.div>
      ) : (
        ...
        >
          ...
          <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col transition-colors" id="panel-original">
            <div className="flex items-center justify-between px-4 py-2 bg-slate-50 dark:bg-slate-950 border-b border-slate-100 dark:border-slate-805">
              <span className="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest">Texto A (Original)</span>
              <button
                onClick={() => copyToClipboard('left', textA)}
                className="flex items-center gap-1.5 px-3 py-1 text-xs text-slate-600 dark:text-slate-350 hover:text-slate-900 dark:hover:text-slate-100 border border-slate-200 dark:border-slate-700 rounded transition-all cursor-pointer font-semibold bg-white dark:bg-slate-800"
              >
                {copiedState === 'left' ? (
                  <>
            ...
            <div className={`p-5 min-h-[160px] max-h-[500px] overflow-y-auto whitespace-pre-wrap break-words leading-relaxed ${fontClass} bg-white dark:bg-slate-900 flex-1 transition-colors`}>
              {changes.map((change, idx) => {
                const isSelected = activeHighlightIndex !== -1 && highlights[activeHighlightIndex]?.globalIdx === idx;
                const selectionStyle = isSelected ? 'bg-indigo-200 ring-2 ring-indigo-400 text-indigo-950 scale-[1.01]' : '';
                ...
                if (change.removed) {
                  return (
                    <span
                      key={idx}
                      data-diff-node={idx}
                      className={`text-red-850 dark:text-red-305 line-through decoration-red-300 dark:decoration-red-700 px-1 py-[1.5px] rounded mx-0.5 inline bg-red-100 dark:bg-red-955/35 ${selectionStyle}`}
                      title="Removido"
                    >
                      {change.value}
                    </span>
                ...
                return <span key={idx} className="text-slate-700 dark:text-slate-300 inline">{change.value}</span>;
              })}
            </div>
          </div>

          {/* Right panel: Modernized (Text B) with ADDITIONS */}
          <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col transition-colors" id="panel-modificado">
            <div className="flex items-center justify-between px-4 py-2 bg-slate-50 dark:bg-slate-950 border-b border-slate-100 dark:border-slate-805">
              <span className="text-[11px] font-bold text-slate-550 dark:text-slate-400 uppercase tracking-widest text-indigo-600 dark:text-indigo-400">Texto B (Modificado)</span>
              <button
                onClick={() => copyToClipboard('right', textB)}
                className="flex items-center gap-1.5 px-3 py-1 text-xs text-indigo-600 dark:text-indigo-400 hover:text-indigo-900 dark:hover:text-slate-100 border border-indigo-150 dark:border-indigo-900/60 rounded transition-all cursor-pointer font-semibold bg-white dark:bg-slate-800"
              >
                {copiedState === 'right' ? (
                  <>
                    <Check className="w-3 h-3 text-emerald-600" />
                ...
                if (change.added) {
                  return (
                    <span
                      key={idx}
                      data-diff-node={idx}
                      className={`text-green-850 dark:text-emerald-305 px-1 py-[1.5px] rounded border-b border-green-250 dark:border-emerald-805 mx-0.5 inline bg-green-100 dark:bg-emerald-955/35 ${selectionStyle}`}
                      title="Adicionado"
                    >
                      {change.value}
                    </span>
